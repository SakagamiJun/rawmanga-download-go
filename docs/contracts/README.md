...
- `localPath`: absolute path to the local chapter source. This may point to a
  chapter directory or to a chapter `.zip` / `.cbz` archive.

### `ReaderPage`

- `sourceURL`: reader image URL served by the local asset handler. This may
  target a filesystem-backed image under `/library-files/` or an archive-backed
  image streamed from `/library-archive/`.
- Archive sidecars use the adjacent stripped-extension naming rule, for example
  `001 - Chapter 1.cbz` pairs with `001 - Chapter 1.klz9-chapter.json`.
